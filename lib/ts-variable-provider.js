...
class TSVariableProvider {
  ...
  async getSymbols(meta) {
    ...
    for (let layer of layers) {
      ...
      let queryCached = this.queryCache[scope];

      if (queryCached) {
        query = queryCached;
      } else {
        query = await languageMode.grammar.createQuery(queryFile);
        this.queryCache[scope] = query;
      }
      ...
      results.push(
        ...this.captureOrganizer(captures, scope)
      );
    }
